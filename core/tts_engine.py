...
# --- CONFIGURACIÓN ---
# Voz local (SAPI5/pyttsx3): usa el ID del registro.
# Nota: las voces "*Neural" NO aplican aquí (esas son de edge-tts).
VOZ_ID_LOCAL_POR_DEFECTO = (
    r"HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Speech\Voices\Tokens\TTS_MS_ES-MX_SABINA_11.0"
)

# Local (pyttsx3): velocidad en WPM (200 ≈ normal). Si pasas "-17%" se convierte automáticamente.
VELOCIDAD_WPM_POR_DEFECTO = 200

# Edge (edge-tts): velocidad estilo "-15%"
VELOCIDAD_EDGE = "-15%"
...
def _parece_voz_edge(voz: str | None) -> bool:
    if not voz:
        return False
    # Las voces de Edge suelen ser como "es-MX-JorgeNeural"
    return "neural" in voz.lower()


def _velocidad_local_a_wpm(velocidad) -> int:
    """Convierte velocidad de entrada a WPM para pyttsx3.

    - int -> se usa tal cual
    - "160" -> 160
    - "-17%" -> 200 * (1 - 0.17) = 166
    - None -> VELOCIDAD_WPM_POR_DEFECTO
    """
...
def _render_wav_subprocess(*, texto: str, ruta_wav: str, voz_id: str | None, rate_wpm: int, timeout_s: int) -> bool:
    """Renderiza WAV en proceso aislado para evitar cuelgues del driver SAPI5."""
...
def _seleccionar_voz_pyttsx3(engine, voz_solicitada: str | None, *, permitir_fallback: bool):
    """Selecciona una voz real disponible en pyttsx3.

    Devuelve (seleccionada_name, seleccionada_id). Si no puede, devuelve (None, None).
    """
    voces = engine.getProperty("voices") or []
    if not voces:
        return None, None

    # 1) Si se pasó un ID exacto (voz_solicitada) y existe en la lista
    if voz_solicitada:
        for v in voces:
            if v.id == voz_solicitada:
                engine.setProperty("voice", v.id)
                return getattr(v, "name", None), v.id
        # 2) Si se pasó un nombre/substring, intenta encontrar coincidencia
    ...
    # 3) Intento preferente: Jorge por ID o por nombre
    candidatos = []
    for v in voces:
        nombre = (getattr(v, "name", "") or "").lower()
        vid = (getattr(v, "id", "") or "").lower()
        ...
        # "Spanish" sin MX/Jorge es demasiado genérico; sólo suma si ya es candidato por MX/Jorge.
    ...
    # 4) Fallback opcional: primera voz
...
def generar_audios(textos, carpeta, voz=None, velocidad=None):
    ...
    - Si `voz` parece de Edge (termina en Neural), usa edge-tts.
    - Si no, usa pyttsx3/SAPI5 (offline) y genera WAV.
    """
    # Ruta Edge (Neural)
    ...
    # Ruta Local (SAPI5) — modo robusto: un subproceso por audio (evita congelamientos del driver SAPI5)
    rutas_generadas = []
